...
object BeanSoupRecipe {
	...
	val recipe = Recipe(
		...
		tasks = listOf(
			...
			CookingTask(
				id = "start-beans",
				title = "Daj fazuľu s vodou na sporák",
				durationSeconds = 60,
				dependsOn = setOf("rinse"),
				resources = uses(COOK, POT, BURNER),
				instruction = "Pridaj cca 800 ml čerstvej vody, bobkový list a 3 guľôčky nového korenia. Zapni sporák.",
			),
			CookingTask(
				id = "wait-for-boil",
				title = "Počkaj, kým voda zovrie",
				durationSeconds = 4 * 60,
				dependsOn = setOf("start-beans"),
				resources = uses(POT, BURNER),
				kind = TaskKind.EVENT,
				instruction = "Kým sa voda zohrieva, môžeš pokračovať v príprave. Keď začne vrieť, potvrď to.",
			...
			CookingTask(
				...
				resources = uses(POT, BURNER),
				kind = TaskKind.WAIT,
				instruction = "Stíš na mierny var. Od potvrdenia varu beží 45-minútový časovač.",
			),
			CookingTask(
				id = "prep-sausage",
			...
			CookingTask(
				...
				instruction = "Pridaj obsah panvice, mrkvu, zemiak, opečenú klobásu a 1/2 kocky zeleninového bujónu. Premiešaj a nechaj ďalej mierne vrieť.",
			),
			CookingTask(
				id = "finish-simmer",
				...
				resources = uses(POT, BURNER),
				kind = TaskKind.WAIT,
				instruction = "Var približne 15–20 minút, kým je zemiak mäkký a fazuľa úplne hotová. Až potom finálne dochucuj a pridávaj ocot.",
			),
			CookingTask(
				id = "season",
				...
				resources = uses(COOK, POT),
				instruction = "Osoľ a okoreň podľa chuti. Vypni oheň, 1/2 ČL majoránu rozmrvi medzi prstami a pridaj ho do polievky. Nakoniec pridaj 1/4 ČL 8 % octu — len trošku, má vytiahnuť chuť, nie spraviť polievku kyslú. Premiešaj a ochutnaj.",
			),
		),
	)
}
