	...
	val recipe = Recipe(
		...
		tasks = listOf(
			...
			CookingTask(
				...
				resources = uses(POT, BURNER),
				kind = TaskKind.EVENT,
				instruction = "Ochutnaj fazuľu. Je už skoro mäkká, bez tvrdého stredu a len s minimálnym odporom? Ak áno, je čas začať opekať klobásu.",
				actionLabel = "ÁNO",
			...
			CookingTask(
				id = "beans-ready",
				title = "Skontroluj, či je fazuľa mäkká",
				durationSeconds = 5 * 60,
				dependsOn = setOf("beans-nearly-ready"),
				resources = uses(POT, BURNER),
				kind = TaskKind.EVENT,
				instruction = "Počas práce s panvicou fazuľu ďalej mierne var. Už vôbec nechrumká a nemá tvrdý stred?",
				actionLabel = "ÁNO",
				retryActionLabel = "NIE",
				retryAfterSeconds = 5 * 60,
			),
			...
			CookingTask(
				...
				resources = uses(POT, BURNER),
				kind = TaskKind.EVENT,
				instruction = "Je fazuľa úplne mäkká, mrkva mäkká a ide vidlička do zemiaka ľahko bez odporu?",
				actionLabel = "ÁNO",
				retryActionLabel = "NIE",
				retryAfterSeconds = 5 * 60,
			),
